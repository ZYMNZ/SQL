...
/*1. Write PL/SQL code that searches for a specific project by its project ID and displays the
...
SELECT* FROM EMPLOYEE WHERE EMP_NO BETWEEN 200 AND 300;

/*3. Calculate the average of all project budgets and compares this value with the budget of
...
SELECT BUDGET INTO VPBUDGET FROM PROJECT WHERE PROJECT_NO = VPNUM;
IF (VPBUDGET < VAVG ) THEN
   UPDATE PROJECT
   SET BUDGET = BUDGET + extra_budget;
   DBMS_OUTPUT.PUT_LINE('PROJECT ' || VPNUM || ' HAS BEEN UPDATED');
END IF;
END;
